...
export function ImageRightAside({
  activeAgent,
  campaignName,
  messages,
  lastResult,
    ...
    <aside className="border-t border-white/10 bg-[#0D1018] xl:border-l xl:border-t-0">
      <div className="flex h-full min-h-[560px] flex-col">
        ...
        <div className="flex-1 space-y-3 overflow-y-auto px-4 py-4">
          ...
          {lastResult && !isBusy ? (
            <div className="mr-4 rounded-md border border-white/10 bg-[#090A0F] p-2">
              <ImageAgentResultBody result={lastResult} />
            </div>
          ) : null}
          {isBusy ? (
            <p className="flex items-center gap-2 text-xs text-white/50">
              <Loader2 className="size-3 animate-spin" /> Generating images…
            </p>
          ) : null}
        </div>
